...
async function fetchFlights(departureAirportTime, departureAirportId) {
  try {
    ...
    const response = await axios.get(`${process.env.PORT}/flights/forWorkers`, {
      params: {
...
app.post('/produce', async (req, res) => {
  ...
  try {
    ...
    const flightResponse = await axios.get(`${process.env.PORT}/flights/${flight_id}`);
    const flightData = flightResponse.data;

    // Obtener los 20 vuelos
    const flightsForWorkers = await fetchFlights(flightData.arrival_airport_id, flightData.arrival_airport_time);

    // Crear un trabajo con los vuelos y agregarlo a la cola
    await queue.add('flightJob', { user_ip, user_id, flightData, flightsForWorkers });
